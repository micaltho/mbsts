 ...
 * 
 *
 * File Path	: ../Source/generic/RXF_Reactive.cpp
 *
...
namespace RXF {
    ...
    Reactive::Reactive(void) : IOxfEventSender(), nullTransitionCount(0U), busy(false), shouldTerminate(false), status(statusInit), currentEvent(nullptr), activeContext(RXF::MainTask::getInstance())
    {
    }
