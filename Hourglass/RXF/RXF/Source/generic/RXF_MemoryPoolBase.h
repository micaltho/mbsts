 ...
 * 
 *
 * File Path	: ../Source/generic/RXF_MemoryPoolBase.h
 *
...
#include <cstddef>
namespace RXF {
    // The class MemoryPoolBase provides the functionality for statically pre-allocated memory pools.
    // This class cannot be instanciated directly, but only through MemoryPool.
    // This class contains the template independant code to keep MemoryPool as slim as possible.
    // Moving type-invariant code into a base class sometimes called code hoisting.
    // It is used internally by the RXF, e.g. for event pools.
    // It is also used by the code generator for preallocated pools for user events or classes ( see Property BaseNumberOfInstances ).
    class MemoryPoolBase {
    protected :
    
        // Argument const std::size_t aSize :
