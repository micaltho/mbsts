 ...
 * 
 *
 * File Path	: ../Source/Monitor/Full/RXF_Event.h
 *
...
namespace RXF {
    ...
    // allocated and freed (via static memory pools), but also static events which do not get destroyed are supported. They can be  
    // used when events are being generated from an interrupt service routine.
    class Event {
    public :
        ...
        inline std::uint16_t getEventID(void) const
        {
            return eventID;
        }
        
        inline void setEventID(const std::uint16_t p_eventID)
        {
            eventID = p_eventID;
        }
        
        inline std::uint8_t getSize(void) const
        {
            return size;
        }
        
        inline void setSize(const std::uint8_t p_size)
        {
            size = p_size;
        }
        
        // Argument const Id_t aId :
        ...
        // 
        explicit inline Event(const Id_t aId = 0, const bool aDeleteAfterConsume = true) : id(aId), deleteAfterConsume(aDeleteAfterConsume), eventID(0U), size(0U), destination(nullptr)
        {
        }
