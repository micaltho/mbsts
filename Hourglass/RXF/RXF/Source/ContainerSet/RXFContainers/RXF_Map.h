 ...
 * 
 *
 * File Path	: ../Source/ContainerSet/RXFContainers/RXF_Map.h
 *
...
namespace RXF {
    ...
    public :
        ...
        inline Map(void) : count( 0U ), END(&emptyEndItem), root( NIL ), first( nullptr ), last( nullptr ), memoryPool(nullptr)
        {
        }
        
        // Argument const Map<Key, T>& other :
        // The map to copy
        inline Map(const Map<Key, T>& other) : count( 0U ), END(&emptyEndItem),  root( NIL ), first( nullptr ), last( nullptr ), memoryPool(nullptr)
        {
            copy( other );
        }
    ...
    private :
        ...
        inline bool isIteratorValid(const Iterator& pos) const
        {
            ...
            {
            	// cannot validate Iterator if no dynamic map without memoryPool is used
            	isValid = true;
            }
